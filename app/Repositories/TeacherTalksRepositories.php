...
class TeacherTalksRepositories
{
    ...
    public static function getTerm($filters)
    {
        ...
        if ( ! is_null($data)) {
            ...
            foreach ($result as $key => $info) {
                if(!in_array($info['Talks_term'], $term)){
                    array_push($term, $info['Talks_term']);
                }
            }
            sort($term);
            return $term;
    ...
    {
        try {
            Te_Talks::where('id', $id)->update($updateData);
